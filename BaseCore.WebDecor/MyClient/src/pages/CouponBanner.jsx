...
const CouponBanner = () => {
    ...
    const [savingId, setSavingId] = useState(null); // id coupon đang lưu
    const [messages, setMessages] = useState({});   // { [couponId]: { type: 'success'|'error', text } }
    ...
    const handleSave = async (coupon) => {
        ...
        if (!coupon.canSave) return;

        setSavingId(coupon.id);
        setMessages(m => ({ ...m, [coupon.id]: null }));
        try {
            const tk = token || localStorage.getItem('token');
            await axios.post(`${API_BASE}/Coupons/save`,
                { couponId: coupon.id },
                { headers: { Authorization: `Bearer ${tk}` } }
            );
            setMessages(m => ({ ...m, [coupon.id]: { type: 'success', text: '✓ Đã lưu vào ví voucher!' } }));
            // Reload để cập nhật trạng thái
            await loadCoupons();
        } catch (err) {
            const msg = err.response?.data?.message || 'Không thể lưu voucher';
            setMessages(m => ({ ...m, [coupon.id]: { type: 'error', text: msg } }));
        } finally {
            setSavingId(null);
        }
    };
